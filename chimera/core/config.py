...
# JSON object: {"alias": "target-model"} e.g. {"claude-4.6-sonnet": "google/gemini-3.1-flash-lite"}
# Applied on top of built-in aliases in all gateway routes.
MODEL_ALIASES_JSON = _env("MODEL_ALIASES_JSON", "")


def _load_model_aliases() -> dict:
    """Parse MODEL_ALIASES_JSON into a dict, skip on bad JSON."""
    import json
    raw = MODEL_ALIASES_JSON.strip()
    if not raw:
        return {}
    try:
        parsed = json.loads(raw)
        if isinstance(parsed, dict):
            return {k.strip(): v.strip() for k, v in parsed.items() if k.strip() and v.strip()}
        return {}
    except Exception:
        return {}


CUSTOM_MODEL_ALIASES: dict = _load_model_aliases()
...
HTTPS_PORT = _env_int("HTTPS_PORT", 8443)  # separate HTTPS port for Claude Desktop
SSL_CERT_FILE = _env("SSL_CERT_FILE", "cert.pem")
SSL_KEY_FILE  = _env("SSL_KEY_FILE",  "key.pem")
