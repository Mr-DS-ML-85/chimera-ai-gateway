...
### 3. Generate TLS Certificates (optional, for Claude Desktop)

Claude Desktop requires HTTPS. Generate a self-signed cert for local dev:

```bash
openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes -subj "/CN=localhost/O=Chimera"
```

> For production, use Let's Encrypt or a CA-signed certificate instead.
...
# Development with HTTPS (for Claude Desktop)
DEV=1 python main.py
...
curl https://localhost:8443/v1/health  # if TLS enabled
```
...
docker run -d --name chimera -p 8000:8000 -p 8443:8443 --env-file .env chimera-gateway
...
> ⚠️ **Claude Desktop requires HTTPS or HTTP on loopback (127.0.0.1).** Use `127.0.0.1` instead of `localhost` for HTTP, or set up TLS (see step 3 above), or run behind a reverse proxy (nginx/Caddy) that terminates HTTPS.

### Quick Start — Claude SDK

```bash
# Set environment (use https://localhost:8443 for TLS, or http://127.0.0.1:8000 for loopback HTTP)
export ANTHROPIC_BASE_URL="http://127.0.0.1:8000"    # HTTP on loopback (Claude Desktop compatible)
export ANTHROPIC_BASE_URL="https://localhost:8443"   # TLS/HTTPS
...
const client = new Anthropic({ 
  baseURL: "http://127.0.0.1:8000",  // Use 127.0.0.1 for Claude Desktop
  apiKey: "your_chimera_api_key"
});
...
### Claude Desktop / Claude Cowork Integration

**No API key required** — Chimera routes to free third-party providers (OpenCode Zen, Google, Groq, etc.) by default.
...
// ~/.claude/settings.json
{
  "env": {
    ...
    "ANTHROPIC_API_KEY": "your_chimera_api_key",
    "ANTHROPIC_MODEL": "auto/fast"
  }
}
```

> **Note:** Use `127.0.0.1` instead of `localhost` — Claude Desktop requires HTTPS or HTTP on loopback.
...
| `gemini-3-flash` | `google/gemini-3-flash-preview-05-20` |
| `gemini-3.1-flash-lite` | `google/gemini-3.1-flash-lite` |
| `claude-4.6-sonnet` / `sonnet-4.6` | `google/gemini-3.1-flash-lite` |
| `opus-4.6` | `auto/reasoning:free` |
| `opus-4.7` | `opencode-zen/minimax-m2.5-free` |
...
# SSL/TLS (for Claude Desktop — requires HTTPS)
# Generate certs: openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes
SSL_CERT_FILE=cert.pem
SSL_KEY_FILE=key.pem

# Custom model aliases (JSON) — no code changes needed
MODEL_ALIASES_JSON={"claude-4.6-sonnet": "google/gemini-3.1-flash-lite"}
```
